...
export default class RadialMenu {
   ...
   constructor({
      ...
      this.textShadowOffsetX = !isNaN(textShadowOffsetX)
         ? textShadowOffsetX
         : 3;

      this.textShadowOffsetY = !isNaN(textShadowOffsetY)
         ? textShadowOffsetY
         : 3;
   ...
   init(id) {
      ...
      this.canvasContext = document.getElementById(id);

      this.w =
         this.outerCircle * 2 +
         this.shadowStyle.blur * 2 +
         this.shadowStyle.offsetX * 2;

      this.h =
         this.outerCircle * 2 +
         this.shadowStyle.blur * 2 +
         this.shadowStyle.offsetY * 2;

      this.canvasContext.style.display = "none";

      this.canvasContext.style.position = "fixed";

      this.canvasContext.style.width = this.w + "px";

      this.canvasContext.style.height = this.h + "px";

      this.canvasContext.width = this.w * this.scale;

      this.canvasContext.height = this.h * this.scale;
      ...
      this.canvasContext.style.zIndex = this.zIndex;
      ...
      const innerMargin =
         (margin * Math.PI * this.innerCircle) / this.outerCircle;

      for (let i = 0; i < this.buttons.length; i++) {
         this.buttons[i]["ini"] =
            (i * this.step + this.rotation + margin) % TWOPI;

         this.buttons[i]["fin"] =
            (this.buttons[i]["ini"] + this.step - 2 * margin) % TWOPI;

         this.buttons[i]["ini_inner"] =
            (i * this.step + this.rotation + innerMargin) % TWOPI;

         this.buttons[i]["fin_inner"] =
            (this.buttons[i]["ini_inner"] + this.step - 2 * innerMargin) %
            TWOPI;
   ...
   draw() {
      const ctx = this.c; //init canvas context...
      ...
      // Draw solid circle with no gaps...
      if (!this.buttonGap) {
      ...
      // Draw each Button section in the Radial Menu with included buttonGap
      for (let i = 0; i < this.buttons.length; i++) {
   ...
   addEvent() {
      ...
      // add click event listener to canvas by id
      this.canvasContext.addEventListener("click", (e) => {
      ...
      if (
         ...
         this.canvasContext.addEventListener("mousemove", (e) => {
   ...
   setPos(x, y) {
      ...
      this.canvasContext.style.left = x + "px";

      this.canvasContext.style.top = y + "px";
   }
   ...
   getButton(xPixel, yPixel) {
      ...
      if (d > this.innerCircle && d < this.outerCircle) {
         ...
         if (!theButton && this.rest) {
            ...
            if (
               a >= this.buttons[this.rest]["ini"] &&
               a <= this.buttons[this.rest]["fin"] + TWOPI
            ) {
